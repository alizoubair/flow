locals {
  ...
  _schemas_root = "${path.module}/schemas"
  _schema_files = fileset(local._schemas_root, "*.yaml")
  _all_schemas = {
    for f in local._schema_files :
    trimsuffix(f, ".yaml") => yamldecode(file("${local._schemas_root}/${f}"))
  }
...
# Gateway IAM role (invokes tool Lambdas)
...
resource "time_sleep" "gateway_iam_propagation" {
  depends_on      = [aws_bedrockagentcore_gateway.mcp]
  create_duration = "5s"
}

# Lambda-backed Gateway targets (one per tool schema)
...
# SSM parameters for Gateway discovery
