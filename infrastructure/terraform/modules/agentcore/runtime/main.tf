...
resource "null_resource" "build_and_wait" {
  ...
  provisioner "local-exec" {
    ...
    interpreter = ["bash", "-c"]
    command     = "if aws ecr describe-images --repository-name \"$ECR_REPO_NAME\" --image-ids imageTag=\"$SOURCE_HASH\" --region \"$AWS_REGION_VAR\" >/dev/null 2>&1; then echo \"Image $ECR_REPO_NAME:$SOURCE_HASH exists, skipping CodeBuild.\"; exit 0; fi; echo \"Starting CodeBuild for $COMPONENT_NAME...\"; START_OUTPUT=$(aws codebuild start-build --project-name \"$PROJECT_NAME\" --region \"$AWS_REGION_VAR\" --query 'build.id' --output text 2>&1); if [ $? -ne 0 ] || [ -z \"$START_OUTPUT\" ] || [ \"$START_OUTPUT\" = \"None\" ]; then echo \"ERROR: Failed to start CodeBuild. Output: $START_OUTPUT\"; exit 1; fi; BUILD_ID=\"$START_OUTPUT\"; echo \"Build ID: $BUILD_ID\"; for i in $(seq 1 90); do STATUS=$(aws codebuild batch-get-builds --ids \"$BUILD_ID\" --region \"$AWS_REGION_VAR\" --query 'builds[0].buildStatus' --output text 2>&1); echo \"  build status: $STATUS\"; case \"$STATUS\" in SUCCEEDED) exit 0;; FAILED|FAULT|STOPPED|TIMED_OUT) echo \"build failed: $STATUS\"; exit 1;; esac; sleep 10; done; echo \"build timeout\"; exit 1"
  }
